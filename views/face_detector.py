...
class FaceDetector:
    def __init__(self):
        """
        Initialize FaceDetector with Haar Cascade face detection only.
        """
        # Initialize Haar Cascade with optimized parameters
        self.face_cascade = cv2.CascadeClassifier(
            cv2.data.haarcascades + "haarcascade_frontalface_default.xml"
        )
    ...
    def detect_faces(self, image):
        """
        Detect faces in the given image using Haar Cascade.
    ...
    def _detect_faces_haar(self, image):
        ...
        faces = self.face_cascade.detectMultiScale(
            gray,
            scaleFactor=1.05,  # Slightly more precise scaling
            minNeighbors=7,  # Higher value reduces false positives
            minSize=(50, 50),  # Minimum face size increased
            flags=cv2.CASCADE_SCALE_IMAGE,
        )
